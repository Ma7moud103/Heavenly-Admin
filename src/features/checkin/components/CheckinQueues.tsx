...
function getGuestName(booking: IRoomBooking) {
  return booking.guest ? `${booking.guest.first_name} ${booking.guest.last_name}` : 'Unknown guest';
}
...
function QueueTable({
  ...
  getPayload: (booking: IRoomBooking) => { bookingId: string; payload: ICreateBookingPayload } | null;
  isUpdating: boolean;
  onRunAction: (args: { bookingId: string; payload: ICreateBookingPayload }) => Promise<void>;
}) {
        ...
        cell: (row) => (
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="truncate text-xs text-[--color-text-muted]">{row.guest?.email || row.guest?.phone || 'No guest contact'}</div>
      ...
      },
      {
        ...
        cell: (row) => (
          <div className="text-sm">
            ...
            <div className="text-xs font-medium text-[--color-text]">{formatCurrency(row.total_price)}</div>
          </div>
        ),
      },
      {
      ...
      >
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="rounded-2xl border border-[--color-border] bg-[--color-bg-raised]/80 px-3 py-2 text-right">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[--color-text-sub]">Queue</p>
...
export function CheckinQueues({ arrivals, departures, canCheckIn, canCheckOut, getActionPayload, isUpdating, onRunAction }: CheckinQueuesProps) {
  return (
    <section className="space-y-3">
      <div>
        <h2 className="text-lg font-semibold">Desk Queues</h2>
        <p className="text-sm text-[--color-text-sub]">Work the arrival and departure lists without leaving the page.</p>
      </div>
