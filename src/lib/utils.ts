...
function formatRelativeTime(value: string) {
  ...
  for (const interval of intervals) {
    if (Math.abs(seconds) >= interval.seconds) {
      return relativeTimeFormatter.format(Math.round(seconds / interval.seconds), interval.unit);
    }
  }
...
function getRoomChanges(oldData: IRoomAuditData | null, newData: IRoomAuditData | null) {
  if (!oldData || !newData) return 'Room details were updated';

  const changes: string[] = [];

  if (oldData.type !== newData.type) {
    changes.push(`type from ${formatLabel(oldData.type, 'Unknown')} to ${formatLabel(newData.type, 'Unknown')}`);
  }

  if (oldData.status !== newData.status) {
    changes.push(`status from ${formatLabel(oldData.status, 'Unknown')} to ${formatLabel(newData.status, 'Unknown')}`);
  }

  if (oldData.base_price !== newData.base_price) {
    changes.push(`price from ${formatCurrency(oldData.base_price)} to ${formatCurrency(newData.base_price)}`);
  }
...
function getBookingChanges(oldData: IRoomBookingAuditData | null, newData: IRoomBookingAuditData | null) {
  ...
  if (oldData.total_price !== newData.total_price) {
    changes.push(`total from ${formatCurrency(oldData.total_price)} to ${formatCurrency(newData.total_price)}`);
  }
...
export function mapAuditToActivity(log: IAuditLog): IRecentActivityItem {
  if (log.table_name === 'rooms') {
    ...
    const roomTitle = newRoom?.title || oldRoom?.title || newRoom?.type || oldRoom?.type || 'Room';
