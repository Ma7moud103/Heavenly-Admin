...
const BookingGuestCell = memo(function BookingGuestCell({ row }: { row: IRoomBooking }) {
  const guestName = row.guest ? `${row.guest.first_name} ${row.guest.last_name}` : 'Unknown';
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0">
        ...
        <div className="truncate text-xs text-[--color-text-muted]">{row.room?.room_type?.name || 'Unknown type'}</div>
      </div>
    </div>
  );
});
...
const BookingActionsCell = memo(function BookingActionsCell({
  ...
  return (
    <div className="flex justify-end">
      <ActionsMenu onDelete={handleDelete} onEdit={handleEdit} />
    </div>
  );
});
...
function BookingsTableComponent({ bookings, onDeleteBooking, onEditBooking }: BookingsTableProps) {
  const columns = useMemo<Column<IRoomBooking>[]>(
    () => [
      ...
      },
      {
        key: 'actions',
        header: '',
        cell: (row) => <BookingActionsCell row={row} onDeleteBooking={onDeleteBooking} onEditBooking={onEditBooking} />,
        className: 'w-12',
      },
    ],
  ...
  return <DataTable data={bookings} columns={columns} emptyMessage="No bookings found" />;
}
