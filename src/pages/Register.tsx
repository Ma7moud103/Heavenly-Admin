...
import { ArrowLeft, ArrowRight, Camera, Check, Mail, MapPin, Phone, ShieldCheck, UserRound, Users, Sparkles, Eye, EyeOff } from 'lucide-react';
...
export default function Register() {
  ...
  const completion = useMemo(() => {
    const values = [form.full_name, form.phone, form.email, form.password, form.country, form.role_name];

    return Math.round((values.filter(Boolean).length / values.length) * 100);
  }, [form]);

  const handleChange = <K extends keyof typeof form>(field: K, value: (typeof form)[K]) => {
    setForm((current: IForm) => ({ ...current, [field]: value }));
  };
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 px-4 py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl relative z-10">
        <div className="text-center mb-12">
          ...
          <h1 className="font-[var(--font-heading)] text-5xl md:text-6xl font-bold tracking-tight text-slate-900 mb-4">Create new account</h1>
                ...
                <div className="flex items-start justify-between gap-6">
                  ...
                  <div className="hidden lg:flex flex-col items-center">
                    <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-xl">
                      {form.avatar_url ? (
                        <img src={form.avatar_url} alt={form.full_name || 'Avatar preview'} className="h-full w-full object-cover" />
                      ) : (
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
                      <div className="flex items-center gap-6">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-2xl font-bold text-slate-900">{form.full_name || 'Unnamed profile'}</p>
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            ...
                            <span
                              className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] ${form.is_active ? 'bg-emerald-100 border border-emerald-200 text-emerald-600' : 'bg-slate-100 border border-slate-200 text-slate-600'}`}
                            >
