...
import { DeleteBookingSheet } from '@/features/bookings/components/actions/DeleteBookingSheet';
import { EditBookingSheet } from '@/features/bookings/components/actions/EditBookingSheet';
import { bookingsFilterReducer, initialBookingsFilters } from '@/features/bookings/filters';
...
function BookingsTableSectionComponent({ bookings, bookingStatuses, guests, rooms, isLoading = false }: BookingsTableSectionProps) {
  ...
  const [filters, dispatch] = useReducer(bookingsFilterReducer, initialBookingsFilters);
  const deferredSearchTerm = useDeferredValue(filters.searchTerm);

  const statusOptions = useMemo(
    () => Array.from(new Set(bookingStatuses.map((status) => status.label || status.name).filter((status): status is string => Boolean(status)))),
    [bookingStatuses],
  );
  ...
  return (
    <>
      <section className="card">
        <BookingsFiltersBar filters={filters} statusOptions={statusOptions} onDispatch={dispatch} />
        <BookingsTable bookings={filteredBookings} onDeleteBooking={handleDeleteBooking} onEditBooking={handleEditBooking} />
      </section>
      ...
      <DeleteBookingSheet open={isDeleteBookingOpen} onOpenChange={handleDeleteBookingOpenChange} booking={selectedBooking} />
    </>
  );
}
